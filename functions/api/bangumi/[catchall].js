// Reference https://github.com/yixiaojiu/bilibili-bangumi-component/blob/main/api/cloudflare.js
const isMock = false;
function p(e) {
...
function d(e) {
  return Object.fromEntries(
    Array.from(e.searchParams.entries()).filter(([, a]) => !!a),
  );
}
function m(e) {
  let a = e.toString();
...
var v = { 1: "anime", 2: "game", 3: "book" },
  R = { 1: "want", 2: "doing", 3: "done" };
function h(e, a) {
  let {
      subjectType: t = "1",
      collectionType: r = "0",
...
function b(e) {
  return c({
    code: 200,
    message: "ok",
    data: { list: [], pageNumber: 1, pageSize: e, total: 0, totalPages: 1 },
  });
}
async function y(e, a) {
  ...
  let i = p({ type: 1, follow_status: t, pn: s, ps: o, vmid: n }),
    ...
    u = await l.json();
  return l.ok
    ? c({ code: 200, message: "ok", data: N(u.data) })
    : c({ code: 502, message: u.message, data: {} });
}
function N(e) {
  return {
    list:
      e?.list?.map((t) => {
        let r = [
          { label: t?.new_ep?.index_show },
          { label: "\u8BC4\u5206", value: t?.rating?.score },
          {
            label: "\u64AD\u653E\u91CF",
            value: t?.stat?.view && m(t.stat.view),
          },
          {
            label: "\u8FFD\u756A\u6570",
          ...
          {
            label: "\u6295\u5E01\u6570",
            value: t?.stat?.coin && m(t.stat.coin),
          },
          {
            label: "\u5F39\u5E55\u6570",
        ...
        return {
          ...
          cover: t.cover,
          url: t.url,
          labels: r.filter((s) => s.label),
        };
      }) ?? [],
    pageNumber: e.pn,
    pageSize: e.ps,
    total: e.total,
    totalPages: Math.ceil(e.total / e.ps),
  };
}
var x = { 1: "2", 2: "4", 3: "1" },
  w = { 0: null, 1: "1", 2: "3", 3: "2" };
async function S(e, a) {
  ...
  return u.ok
    ? c({
        code: 200,
        message: "ok",
        data: L(f, { pageNumber: Number(o), pageSize: Number(n) }),
      })
    : c({ code: 502, message: f.description, data: {} });
}
function L(e, a) {
  return {
    list:
      e?.data?.map((r) => {
        let s = r?.subject,
          o = [
            { label: s?.eps && `${s.eps}\u8BDD` },
            { label: "\u8BC4\u5206", value: s?.score },
            { label: "\u6392\u540D", value: s?.rank },
    ...
    ...a,
    total: e.total,
    totalPages: Math.ceil(e.total / a.pageSize),
  };
}
function j(e) {
...
function g(e) {
  return (
    e.headers.set("Access-Control-Allow-Origin", "*"),
    e.headers.set("Access-Control-Max-Age", "86400"),
    e
  );
}
var F = {
  async fetch(e, a) {
    ...
    return t.pathname.endsWith("bilibili")
      ? g(await y(r, a))
      : t.pathname.endsWith("bgm")
        ? g(await S(r, a))
        : t.pathname.endsWith("custom")
          ? g(h(r, s))
          : Response.json(
...
export function onRequest(ctx) {
  return F.fetch(ctx.request, ctx.env);
}
